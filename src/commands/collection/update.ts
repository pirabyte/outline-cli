...
export async function runCollectionUpdate(ctx: CommandContext): Promise<CommandExecution> {
  ...
  const response = await ctx.client.post("collections.update", request);
  return { method: "collections.update", request, response };
}
